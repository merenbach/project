...
class Command(BaseCommand):
    ...
    def do_compress(self, infile, outfile):
        os.system('gzip --best --stdout %s > %s' % (infile, outfile))
    ...
    def do_postgresql_backup(self, outfile):
        ...
        #env = {'PGPASSFILE':'/home/merenbach/.pgpass'}
        env = dict(PGPASSWORD=self.passwd)
        ...
        #putenv('PGPASSWORD', self.passwd)
        #cmd = ['pg_dump']
        #cmd.extend(args)
        outf = open(outfile, 'wb')
        pipe = Popen(args, shell=False, stdin=PIPE, stdout=outf, env=env)
        #if self.passwd:
        #    pipe.communicate('%s\n' % self.passwd)
        #    #pipe.stdin.flush()
        #    #pipe.stdin.write('%s\n' % self.passwd)
